...
export class CubeMapRenderer {
    ...
    p.enableAttribList(this.attribList, () => {
      ...
      glUtils.enable(gl, [gl.BLEND], () => {
        ...
        this.indexBuffer.bind(() => {
          ...
          gl.drawElements(gl.TRIANGLE_STRIP, this.indexBuffer.length, gl.UNSIGNED_SHORT, 0)
  ...
  private buildArray() {
    const { position, index } = buildArray(10)
...
function buildArray(nDiv = 5) {
  const permutations = [[2, 1], [2, -1], [0, 1], [0, -1], [1, 1], [1, -1]]
  ...
  for (const p of permutations) {
    for (let i = 0; i < nDiv; ++i) {
      const t = -1 + 2 * (i / (nDiv - 1))
      for (let j = 0; j < nDiv; ++j) {
        const u = -1 + 2 * (j / (nDiv - 1))
        const r = Math.sqrt(1 + t * t + u * u)
        const x = (p[0] + 1) % 3
        const y = (p[0] + 2) % 3
        const z = p[0]
        const w = {
        ...
        position.push(w[0], w[1], w[2])
      }
    }
    for (let i = 0; i < nDiv - 1; ++i) {
      index.push(offset + i * nDiv)
      for (let j = 0; j < nDiv; ++j) {
        index.push(
          offset + i * nDiv + j,
          offset + (i + 1) * nDiv + j,
      ...
      index.push(offset + (i + 1) * nDiv + (nDiv - 1))
    }
    offset += nDiv * nDiv
  }
