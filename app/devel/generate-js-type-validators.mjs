...
function main() {
  const schemata = [
    { $id: 'HashState', ...extractSchema(root, ['HashState']) },
    { $id: 'StorageState', ...extractSchema(root, ['StorageState']) },
    { $id: 'BaseAction', ...extractSchema(root, ['BaseAction']) },
    ...Object.keys(root.properties['Actions']?.properties ?? {}).map(type => ({
      $id: `Actions/${type}`.replace(/\//g, '$'),
      ...extractSchema(root, ['Actions', type]),
    }))
  ]
  const ajv = new Ajv({
    schemas: schemata,
  ...
  writeFileSync("./src/app/typeGuard/typeValidators.js", moduleCode)
}
